...
class PgStore {
  ...
  */
  storeActiveShop = async (data: { shop: string; scope: string; accessToken: string }) => {
  ...
  createLocalContract = async (shop: string, contract: any) => {
    try {
      ...
      }', '${interval}', '${intervalCount}', '${JSON.stringify(contract)}') RETURNING *;
      `;
  ...
  updateLocalContract = async (shop: string, contract: any) => {
    try {
      ...
      const intervalCount = contract.billingPolicy.intervalCount;
      const query = `
        UPDATE subscription_contracts SET status = '${contract.status}', next_billing_date = '${
        contract.nextBillingDate
  ...
  */
  updateSubscriptionContractAfterSuccess = async (shop: string, token: string, body: any) => {
    body = JSON.parse(body);
  ...
  */
  ...
  /*
    Get subscription contracts by status
  */

  getSubscriptionsByStatus = async (shop: string, body: string) => {
