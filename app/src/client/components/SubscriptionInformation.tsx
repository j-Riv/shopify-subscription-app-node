    ...
    <Card title="Subscription" sectioned>
      <Container>
        <Button
          plain
          onClick={() =>
            adminRedirect(`/orders/${data.subscriptionContract.originOrder.legacyResourceId}`)
          }
        >
          View Original Order
        </Button>
            ...
            data.subscriptionContract.lines.edges.map((line) => (
              <div key={line.node.id} className="product">
                ...
                <div className="information">
                  <p>
                    {line.node.title}
                    {line.node.variantTitle ? ` - ${line.node.variantTitle}` : ''}
                  </p>
                  <p>{`$${formatMoney(line.node.currentPrice.amount)} x ${line.node.quantity}`}</p>
                </div>
              </div>
            ))
          ) : (
