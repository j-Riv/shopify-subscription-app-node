...
import { GET_SELLING_PLAN_GROUP_BY_ID, DELETE_SELLING_PLAN_GROUP } from '../handlers';
...
function SellingPlanGroup() {
  ...
  // Get Data
  const { loading, error, data, refetch } = useQuery(GET_SELLING_PLAN_GROUP_BY_ID, {
    variables: {
      id: id,
    },
    onCompleted: (data) => {
  ...
  if (data.sellingPlanGroup) {
    return (
      <Page
        breadcrumbs={[{ content: 'All Selling Plan Groups', onAction: appRedirect }]}
          ...
          <Layout>
            <Layout.Section>
              ...
              <Card title="Selling Plans" sectioned>
                <ul>
                  {data.sellingPlanGroup.sellingPlans.edges.map((sellingPlan: SellingPlan) => {
                    return (
                      <li key={sellingPlan.node.id}>
                        {sellingPlan.node.name} <em>({sellingPlan.node.id})</em>
                      </li>
                    );
                  })}
                </ul>
              ...
              <Card title="Products" sectioned>
                <TextStyle variation="subdued">
                  <Message>
                    To add products to this Selling Plan Group, please go to the product page.
                  </Message>
                </TextStyle>
                {data.sellingPlanGroup.products.edges.map((product: ShopifyProduct) => {
                  return (
              ...
              <Card title="Variants" sectioned>
                <TextStyle variation="subdued">
                  <Message>
                    To add variants to this Selling Plan Group, please go to the product page and
                    select the variant.
                  </Message>
                </TextStyle>
    ...
    return (
      <Page>
        <ErrorState err={`Selling Plan Group (${id}) Not Found!`} />
      </Page>
    );
  }
}
