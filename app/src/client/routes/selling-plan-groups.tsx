...
import { GET_ALL_SELLING_PLAN_GROUPS, DELETE_SELLING_PLAN_GROUP } from '../handlers';
...
function RemoveButton(props: { id: string; toggleActive: () => void }) {
  ...
  const handleClick = (sellingPlanId: string) => {
    deleteSellingGroup({
      variables: {
        id: sellingPlanId,
      },
...
function SellingPlanGroups() {
  ...
  const toastMarkup = active ? <Toast content="Removed" onDismiss={toggleActive} /> : null;
        ...
        <Card sectioned>
          {data && data.sellingPlanGroups.edges.length > 0 ? (
            <Table
              ...
              rows={data.sellingPlanGroups.edges.map((group: SellingPlanGroup) => {
                return [
                  group.node.name,
                  group.node.summary,
                  <Actions key={group.node.id}>
                    <RemoveButton id={group.node.id} toggleActive={toggleActive} />
                    <Button
                      plain
                      onClick={() => appRedirect(`/selling-plan-group/?id=${group.node.id}`)}
                    >
                      View
                    </Button>
                  </Actions>,
